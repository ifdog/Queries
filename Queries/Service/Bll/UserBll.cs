...
namespace Service.Bll
{
    ...
    {
	    private readonly LiteDal<UserDbModel> _userDal = new LiteDal<UserDbModel>();

	    public ResultUserModel Register(UserModel user)
	    {
			if (_userDal.Find($"X@User.UserName:{user.UserName}").Any())
	    ...
	    public ResultUserModel Login(UserModel user)
	    {
		    ...
		    var u = _userDal.Find($"X@User.UserName:{user.UserName}").FirstOrDefault();
